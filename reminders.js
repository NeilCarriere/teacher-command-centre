(function(){
  ...
  function install(){
    ...
    style.textContent=`
      .reminder-sticky{position:relative;background:linear-gradient(145deg,#ffe77d,#f5cf56);color:#1f241f;border-radius:3px 4px 10px 3px;box-shadow:0 8px 18px rgba(0,0,0,.28);padding:16px 15px 14px;font-family:"Segoe Print","Comic Sans MS",cursive;transform:rotate(.35deg)}
      ...
      .reminder-form{display:grid;grid-template-columns:140px minmax(0,1fr) auto;gap:7px;align-items:center;margin-bottom:11px}.reminder-form input{background:rgba(255,255,255,.48);border:1px solid rgba(31,36,31,.28);color:#1f241f;padding:7px 8px}.reminder-form button{border:1px solid rgba(31,36,31,.3);background:rgba(31,36,31,.12);color:#1f241f;border-radius:8px;padding:7px 9px;font-weight:800;cursor:pointer}
      .reminder-list{display:grid;gap:7px}.reminder-item{display:grid;grid-template-columns:auto minmax(0,1fr) auto;gap:8px;align-items:start;padding:7px 0;border-top:1px dashed rgba(31,36,31,.28)}.reminder-item:first-child{border-top:0}.reminder-date{font-family:"Trebuchet MS",sans-serif;font-size:11px;font-weight:900;white-space:nowrap;padding-top:2px}.reminder-text{font-size:13px;line-height:1.3}.reminder-actions{display:flex;gap:4px}.reminder-actions button{border:0;background:transparent;color:#1f241f;cursor:pointer;padding:2px 3px;font-size:14px}.reminder-overdue .reminder-date{color:#9b1c1c}.reminder-today .reminder-date{color:#205c35}.reminder-empty{font-size:12px;opacity:.7;padding:5px 0}.reminder-done{opacity:.48;text-decoration:line-through}
      .reminder-manager{margin-top:14px}.reminder-manager .reminder-sticky{max-width:780px;transform:none}
      @media(max-width:700px){.reminder-form{grid-template-columns:1fr}.reminder-form>*{width:100%}}
    `;
    ...
    function makeSticky(compact=false){const wrap=d.createElement('div');wrap.className='reminder-sticky';wrap.innerHTML='<h3>📌 Reminders</h3><div class="reminder-form"><input type="date" class="reminder-date-input"><input type="text" class="reminder-text-input" placeholder="e.g. Marks due Friday"><button class="reminder-add">＋ Add</button></div><div class="reminder-list"></div>';return wrap}
    const recentPanel=recent.closest('.panel');
    const dashboardSticky=makeSticky(true);dashboardSticky.id='dashboardReminderSticky';
    ...
    if(notesView){const shell=notesView.querySelector('.notes-shell')||notesView.querySelector('.panel');notesSticky=makeSticky(false);const holder=d.createElement('div');holder.className='reminder-manager';holder.appendChild(notesSticky);if(shell)shell.appendChild(holder)}
