(function(){
  ...
  function wireAddForm(d,w){
    ...
    btn.onclick=e=>{e.preventDefault();e.stopPropagation();const cls=activeClass(d,w),name=d.getElementById('trackerAssignmentName')?.value.trim()||'',assigned=d.getElementById('trackerAssignedDate')?.value||'',due=d.getElementById('trackerDueDate')?.value||'';if(!name){alert('Enter an assignment name.');return}const data=readData(w);if(!data.classes)data.classes={};if(!data.classes[cls])data.classes[cls]={assignments:[]};if(!Array.isArray(data.classes[cls].assignments))data.classes[cls].assignments=[];const a={id:nowId(),name,assigned,due,maxMark:100,grading:'levels',archived:false,students:{}};(w.ROSTERS?.[cls]||[]).forEach(n=>a.students[n]={submitted:false,mark:'',achievement:'',note:''});data.classes[cls].assignments.push(a);saveData(w,data);reloadTo(w,cls,a.id)};
  }
  ...
  function decorateCards(d,w){
    ...
    d.querySelectorAll('#assignmentList .assignment-card').forEach(card=>{
      const ref=card.querySelector('[data-open-assignment],[data-archive-assignment]');if(!ref)return;const id=ref.dataset.openAssignment||ref.dataset.archiveAssignment;const a=findAssignment(data,cls,id,'');if(!a)return;if(a.grading!=='levels'){a.grading='levels';dirty=true}
  ...
  function decorateDetail(d,w){
    ...
    const meta=detail.querySelector('.assignment-meta');if(meta)meta.textContent=(w.ROSTERS?.[cls]||[]).filter(n=>!a.students?.[n]?.submitted).length+' students outstanding · achievement levels R–4+';
    ...
    detail.querySelectorAll('[data-mark-student]').forEach(input=>{
      ...
      select.onchange=()=>{const fresh=readData(w),fa=findAssignment(fresh,cls,a.id,a.name);if(!fa)return;if(!fa.students[student])fa.students[student]={submitted:false,mark:'',achievement:'',note:''};const fr=fa.students[student],v=select.value;fr.achievement=v;fr.mark=v;if(v)fr.submitted=true;fa.grading='levels';saveData(w,fresh);const row=select.closest('tr'),cb=row?.querySelector('[data-submit-student]');if(cb&&v)cb.checked=true;if(row){row.classList.toggle('submitted-row',!!fr.submitted);row.classList.toggle('missing-row',!fr.submitted);const cells=row.querySelectorAll('td');if(cells.length)cells[cells.length-1].textContent=fr.submitted?'Handed in':'Outstanding'}decorateCards(d,w)};input.replaceWith(select)});
    detail.querySelectorAll('[data-submit-student]').forEach(cb=>{if(cb.dataset.levelSubmit==='1')return;cb.dataset.levelSubmit='1';cb.onchange=()=>{const student=cb.dataset.submitStudent,fresh=readData(w),fa=findAssignment(fresh,cls,a.id,a.name);if(!fa)return;if(!fa.students[student])fa.students[student]={submitted:false,mark:'',achievement:'',note:''};const fr=fa.students[student];fr.submitted=cb.checked;if(!cb.checked){fr.achievement='';fr.mark='';const sel=cb.closest('tr')?.querySelector('.assignment-level-select');if(sel)sel.value=''}fa.grading='levels';saveData(w,fresh);const row=cb.closest('tr');if(row){row.classList.toggle('submitted-row',cb.checked);row.classList.toggle('missing-row',!cb.checked);const cells=row.querySelectorAll('td');if(cells.length)cells[cells.length-1].textContent=cb.checked?'Handed in':'Outstanding'}decorateCards(d,w)}});
    saveData(w,data);
  }
