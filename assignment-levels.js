(function(){
  ...
  function addFormNote(d){
    ...
    const help=d.querySelector('#assignmentTrackerView .history-help');if(help&&/marks/i.test(help.textContent))setText(help,'Track active assignments, submissions, and rubric achievement levels separately for each class. Changes save immediately on this device.');
  }
  ...
  function wireAddForm(d,w){
    ...
    btn.onclick=e=>{e.preventDefault();e.stopPropagation();const cls=activeClass(d,w),name=d.getElementById('trackerAssignmentName')?.value.trim()||'',assigned=d.getElementById('trackerAssignedDate')?.value||'',due=d.getElementById('trackerDueDate')?.value||'';if(!name){alert('Enter an assignment name.');return}const data=readData(w);if(!data.classes)data.classes={};if(!data.classes[cls])data.classes[cls]={assignments:[]};if(!Array.isArray(data.classes[cls].assignments))data.classes[cls].assignments=[];const a={id:nowId(),name,assigned,due,maxMark:4,grading:'levels',archived:false,students:{}};(w.ROSTERS?.[cls]||[]).forEach(n=>a.students[n]={submitted:false,mark:'',achievement:'',note:''});data.classes[cls].assignments.push(a);saveData(w,data);reloadTo(w,cls,a.id)};
  }
  ...
  function decorateDetail(d,w){
    ...
    setText(detail.querySelector('.assignment-meta'),metaText);
  ...
  function install(){
    ...
    if(d.body.dataset.assignmentLevelsInstalled!=='2'){
      d.body.dataset.assignmentLevelsInstalled='2';
